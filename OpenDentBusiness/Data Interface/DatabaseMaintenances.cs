...
namespace OpenDentBusiness {
	public class DatabaseMaintenances {
		...
		[DbmMethodAttr(IsCanada=true,HasBreakDown=true)]
		...
		public static string ProcedurelogDeletedWithAttachedIncome(bool verbose, DbmMode modeCur, long patNum) {
			...
			List<ClaimProc> listClaimProcsForDeletedProcs=ClaimProcs.GetForProcs(listDeletedProcNums).FindAll(x=>ListTools.In(x.Status,listPaidStatus));
			...
			List<Procedure> listProceduresDeletedWithIncome=listDeletedProceduresForFam.FindAll(x=>ListTools.In(x.ProcNum,listFKToIncome));//List of procedures with income.
		...
		public static string ProcedurelogWithInvalidAptNum(bool verbose,DbmMode modeCur) {
			...
			switch(modeCur) {
				...
				case DbmMode.Fix:
					...
					foreach(int procNum in dictProcNumsForPlannedApts.Keys) {
...
HAVING cnt>1";
		...
		public static string PatientMissing() {
			...
			string command="SELECT PatNum FROM (\r\n"
				+string.Join("\r\nUNION\r\n",listMissingPats.Select(x =>
					$"SELECT DISTINCT({POut.String(x.ColumnName)}) "+
					$"FROM {POut.String(x.TableName)} "+
			...
			for(int i=0;i<listMissingPatNums.Count;i++) {
				...
				if(listMissingPatNums[i]==0) {
					for(int j=0;j<listMissingPats.Count;j++) {
						command=$"SELECT COUNT({POut.String(listMissingPats[j].ColumnName)}) " +
							$"FROM {POut.String(listMissingPats[j].TableName)} " +
							$"WHERE {POut.String(listMissingPats[j].ColumnName)}=0";
						if(Db.GetCount(command)=="0") {
							continue;
						}
						command=$"UPDATE {POut.String(listMissingPats[j].TableName)} " +
							$"SET {POut.String(listMissingPats[j].ColumnName)}={POut.Long(tempPat.PatNum)} " +
							$"WHERE {POut.String(listMissingPats[j].ColumnName)}=0";
						Db.NonQ(command);
					}
				}
		...
		private class PatientMissingTableHelper {
			...
			public PatientMissingTableHelper(string tableName,string columnName) {
				TableName=tableName;

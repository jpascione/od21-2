...
namespace OpenDentBusiness{
	...
	public class Etranss {
			...
			if(Xobj!=null) {//Is a correctly formatted X12 message.
				if(Xobj.IsAckInterchange()) {
					...
					for(int i=0;i<listEtrans.Count;i++) {
						...
						if(IsEtransAutomatable(listCarriers,list835s[i],isFullyAutomatic:true)) {
		...
		public static bool IsEtransAutomatable(List<Carrier> listCarriersForClaims,X835 x835,bool isFullyAutomatic) {
			//No need to check RemotingRole; no call to db.
			...
			string payerName=x835.PayerName;

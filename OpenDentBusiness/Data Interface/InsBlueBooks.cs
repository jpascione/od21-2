...
namespace OpenDentBusiness {
	...
	public class InsBlueBooks {
		...
		public static List<InsBlueBook> GetAllForCarrierGroupLimitByDateAndClaimType(long carrierGroupName,DateTime dateLimit,string claimType) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<InsBlueBook>>(MethodBase.GetCurrentMethod(),carrierGroupName,dateLimit,claimType);
			}
			string command=$@"
				SELECT insbluebook.*
				FROM carrier
		...
		public static List<InsBlueBook> GetAllForCarrierLimitByDateAndClaimType(long carrierNum,DateTime dateLimit,string claimType) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<InsBlueBook>>(MethodBase.GetCurrentMethod(),carrierNum,dateLimit,claimType);
			}
			string command=$@"
				SELECT insbluebook.*
				FROM insbluebook 
	...
	public class BlueBookEstimateData {
		...
		public BlueBookEstimateData(List<InsPlan> listInsPlans,List<InsSub> listInsSubs,List<PatPlan> listPatPlans) {
		...
		private void Initialize() {
			...
			if(InsPlanPrimaryDental==null
				...
				|| InsPlanPrimaryDental.FeeSched!=0
			...
			GetInsBlueBooksNeeded(oldestLimit);
		}
		...
		private void GetInsBlueBooksNeeded(DateTime oldestLimit) {
			//No need to check RemotingRole; no call to db.
			if(CarrierPri.CarrierGroupName==0) {
				ListInsBlueBooks=InsBlueBooks.GetAllForCarrierLimitByDateAndClaimType(CarrierPri.CarrierNum,oldestLimit,"P");
			}
			else {
				ListInsBlueBooks=InsBlueBooks.GetAllForCarrierGroupLimitByDateAndClaimType(CarrierPri.CarrierGroupName,oldestLimit,"P");
			}
		}

...
namespace OpenDentBusiness {
	...
	public class Patients {
		...
		public static DataTable GetPatientsWithFirstLastAppointments(List<PatientStatus> listPatStatus,bool doExcludePatsWithFutureAppts,
			...
			,int numDays=-1, List<long> listPatNums=null)
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetTable(MethodBase.GetCurrentMethod(),listPatStatus,doExcludePatsWithFutureAppts,listClinicNums,ageFrom,ageTo,dateExcludeSeenSince
					,dateExcludeNotSeenSince,listBillingType,contactMethod,numDays,listPatNums);
			}
			...
			string command=@$"SELECT patient.PatNum,patient.FName,patient.LName,patient.Email,patient.PatStatus,patient.PreferContactMethod,patient.ClinicNum,
				patient.Birthdate,apt.DateTimeLastApt,DATE(apt.DateTimeNextApt) DateTimeNextApt,a.AptNum NextAptNum,recentEmail.DateTimeLastEmail
				FROM patient 
				LEFT JOIN (
				...
				LEFT JOIN (
					SELECT MAX(DATE(promotionlog.DateTimeSent)) DateTimeLastEmail,promotionlog.PatNum
					FROM promotionlog
					GROUP BY promotionlog.PatNum
				)recentEmail ON recentEmail.PatNum=patient.PatNum
			...
			if(numDays > -1) {
				command+=$"AND (recentEmail.DateTimeLastEmail < {POut.Date(dateExcluding)} " 
					+$"OR recentEmail.DateTimeLastEmail IS NULL) ";
			}
		...
		public static string ReplacePatient(string message,Patient pat,bool isHtmlEmail=false) {
			...
			ReplaceTags.ReplaceOneTag(template,"[LNameLetter]",pat.LName.Substring(0,1).ToUpper(),isHtmlEmail);
			ReplaceTags.ReplaceOneTag(template,"[NameF]",pat.FName,isHtmlEmail);
			ReplaceTags.ReplaceOneTag(template,"[NameFL]",Patients.GetNameFL(pat.LName,pat.FName,"",""),isHtmlEmail);
		...
		/// <summary>Gets ten of the most useful fields from the db for the given patnum.  If invalid PatNum, returns new Patient rather than null.</summary>
		public static Patient GetLim(long patNum) {
			...
			string command= 
				"SELECT PatNum,LName,FName,MiddleI,Preferred,CreditType,Guarantor,HasIns,SSN,Birthdate " 
				+"FROM patient "
		...
		///<summary>Key=patNum, value=formatted name. Stop using this.  Switch to GetPatientNameList().</summary>
		[Obsolete()]
		...
		public static List<PatientName> GetPatientNameList(List<long> listPatNums) {
			...
			for(int i = 0;i<listPatients.Count;++i) {
				...
				patientName.Name=listPatients[i].GetNameFL();
			...
			bool excludeNoTil=false,bool excludeNotBilledSince=false,bool isFinanceBilling=false)
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<PatAging>>(MethodBase.GetCurrentMethod(),age,lastStatement,billingNums,excludeAddr,excludeNeg,excludeLessThan,
					...
					excludeNoTil,excludeNotBilledSince,isFinanceBilling);
			}
			if(DataConnection.DBtype!=DatabaseType.MySql) {
		...
		public static List<PatComm> GetPatComms(List<long> patNums,Clinic clinic,bool isGetFamily=true) {
			...
			command="SELECT PatNum, PatStatus, PreferConfirmMethod, PreferContactMethod, PreferRecallMethod, PreferContactConfidential, "
				+"TxtMsgOk,HmPhone,WkPhone,WirelessPhone,Email,FName,LName,Preferred,Guarantor,Language,Birthdate,";
			if(clinic!=null) {

...
namespace OpenDentBusiness{
	...
	public class PaySplits {
		...
		public static List<PaySplit> GetPaySplitsFromProc(long procNum,bool onlyHiddenUnearned=false) {
			//No need to check RemotingRole; no call to db.
			return GetPaySplitsFromProcs(new List<long>() { procNum }, onlyHiddenUnearned);
		}
		...
		public static List<PaySplit> GetPaySplitsFromProcs(List<long> listProcNums,bool onlyHiddenUnearned=false) {
			...
			if(onlyHiddenUnearned) {
				List<long> listHiddenUnearnedTypes = GetHiddenUnearnedDefNums();
				if(listHiddenUnearnedTypes.Count > 0) {
					command+=$" AND UnearnedType IN({string.Join(",",listHiddenUnearnedTypes)})";
				}
				else {
					return new List<PaySplit>();
                }

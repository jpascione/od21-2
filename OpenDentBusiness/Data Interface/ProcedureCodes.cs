	...
	public class ProcedureCodes {
		...
		public static int ResetADAdescriptionsAndAbbrs(List<ProcedureCode> codeList) {
			...
			for(int i=0;i<codeList.Count;i++) {
				...
				bool isAbbrDescMatch=code.AbbrDesc==codeList[i].AbbrDesc;
				if(!isDescriptMatch || !isAbbrDescMatch) {//Only increments one time for each code if there are changes necessary.
					count++;
				}
				...
				if(!isAbbrDescMatch) {//Update abbreviation.

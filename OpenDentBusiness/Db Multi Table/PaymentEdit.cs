	...
	public class PaymentEdit {
			...
			bool doIncludeTreatmentPlanned=false)
		{
			...
			return ConstructAndLinkChargeCredits(listPatNums,patNum,new List<PaySplit>(),new Payment(),new List<AccountEntry>(),
				isIncomeTxfr:isIncomeTxfr,doIncludeTreatmentPlanned:doIncludeTreatmentPlanned);
		}
			...
			List<AccountEntry> listCharges)
		{
			...
			List<FauxAccountEntry> listOffsetDebits=listPayPlanAccountEntries.FindAll(x => x.IsOffset);
			foreach(FauxAccountEntry offsetEntry in listOffsetDebits) {
				List<FauxAccountEntry> listRelatedDebits=listPayPlanAccountEntries.FindAll(x => !x.IsOffset
					&& x.IsAdjustment==offsetEntry.IsAdjustment
					&& x.ChargeType==offsetEntry.ChargeType
					&& x.ProcNum==offsetEntry.ProcNum
					&& x.AdjNum==offsetEntry.AdjNum
					&& x.PayPlanNum==offsetEntry.PayPlanNum);
				foreach(FauxAccountEntry relatedEntry in listRelatedDebits) {
					if(offsetEntry.AmountEnd>=0) {
						break;
					}
					if(relatedEntry.AmountEnd<=0) {
						continue;
					}
					//The offsetting debit still has a negative value that needs to be removed from related debits.
					decimal amountRemove=Math.Min(Math.Abs(offsetEntry.AmountEnd),Math.Abs(relatedEntry.AmountEnd));
					offsetEntry.AmountEnd+=amountRemove;
					relatedEntry.AmountEnd-=amountRemove;
				}
			}
			#endregion
			...
			foreach(AccountEntry accountEntry in listAllocateEntries.FindAll(x => CompareDecimal.IsGreaterThanZero(x.AmountEnd))) {
				...
				foreach(AccountEntry unearnedEntry in listUnearnedEntries.OrderByDescending(x => x.ProvNum==accountEntry.ProvNum)) {
					...
					listPaySplits.Add(new PaySplit() {
						...
						PayPlanChargeNum=unearnedEntry.PayPlanChargeNum,
						PayPlanDebitType=unearnedEntry.PayPlanDebitType,
					...
					listPaySplits.Add(new PaySplit() {
						...
						PayPlanChargeNum=accountEntry.PayPlanChargeNum,
						PayPlanDebitType=accountEntry.PayPlanDebitType,
			...
			bool isManual=false)
		{
			...
			split.ProvNum=charge.ProvNum;
			split.PayPlanChargeNum=charge.PayPlanChargeNum;
			split.PayPlanDebitType=charge.PayPlanDebitType;
			split.PayPlanNum=charge.PayPlanNum;
			split.ClinicNum=charge.ClinicNum;
		...
		private static void PreprocessPayPlanSplits(ref List<AccountEntry> listAccountEntries,ref IncomeTransferData incomeTransferData,DateTime datePay) {
			...
			foreach(var kvp in dictLinkedPayPlanSplits) {
				...
				List<PaySplit> listPaySplits=CreateUnearnedTransfer(offsetAmt,
					...
					payPlanNum:listPayPlanEntries.First().PayPlanNum,
					datePay:datePay,
					payPlanDebitType:PayPlanDebitTypes.Principal);//There is no such thing as 'interest' payment splits that are associated to production.
				AccountEntry accountEntryOffset=new AccountEntry(listPaySplits[0]);
				AccountEntry accountEntryUnearned=new AccountEntry(listPaySplits[1]);
				//Zero out the AmountEnd field for every negative account entry so that they are not transferred again.
				listPayPlanEntries.ForEach(x => x.AmountEnd=0);
		...
		private static IncomeTransferData CreateTransferHelper(AccountEntry posCharge,AccountEntry negCharge,long guarNum,DateTime datePay) {
			...
			posSplit.PayPlanChargeNum=posCharge.PayPlanChargeNum;
			posSplit.PayPlanDebitType=posCharge.PayPlanDebitType;
			...
			negSplit.PayPlanChargeNum=negCharge.PayPlanChargeNum;
			negSplit.PayPlanDebitType=negCharge.PayPlanDebitType;
		...
		private static List<PaySplit> CreateUnearnedTransfer(AccountEntry accountEntry,ref IncomeTransferData incomeTransferData,DateTime datePay) {
			...
			return CreateUnearnedTransfer(accountEntry.AmountEnd,accountEntry.PatNum,accountEntry.ProvNum,accountEntry.ClinicNum,ref incomeTransferData,
				...
				payPlanChargeNum:accountEntry.PayPlanChargeNum,payPlanDebitType:accountEntry.PayPlanDebitType);
		}
		...
		public class IncomeTransferData {
				...
				,double transferAmtOverride=0)
			{
				...
				offsetSplit.PayPlanChargeNum=parentSplit.PayPlanChargeNum;
				offsetSplit.PayPlanDebitType=parentSplit.PayPlanDebitType;
				offsetSplit.PayPlanNum=parentSplit.PayPlanNum;

	...
	public class EdgeExpress {
		...
		public class RCM {
			...
			public static bool IsRCMRunning {
				get {
					...
					try {
						isRCMRunning=(ServicesHelper.GetServiceByServiceName("RCM",isCaseSensitive:false)!=null);
					}
					catch(Exception ex) {
						ex.DoNothing();
					}
		...
		public class CNP {
			...
			private static XWebResponse CreateEdgeExpressXWebResponse(string result,EdgeExpressTransType edgeExpressTransactionType,bool isDirectPay) {
				...
				else if(ListTools.In(edgeExpressTransactionType,EdgeExpressTransType.CreditVoid,
			...
			public static XWebResponse GetUrlForPaymentPage(long patNum,string payNote,double amount,bool createAlias,
				CreditCardSource ccSource,bool isWebPayment,string alias="",bool allowDuplicates=false)
			{
			...
			public static XWebResponse ProcessPaymentDirect(long patNum,string payNote,double amount,CreditCardSource ccSource,bool isWebPayment,string alias,bool allowDuplicates) {
				...
				try {
					XWebResponse response=SendEdgeExpressRequest(patNum,EdgeExpressTransType.CreditSale,_edgeExpressDirectPayUrl,isWebPayment,amount,
						doCreateAlias:false,alias:alias,allowDuplicates:allowDuplicates);
					response.Amount=amount;
					response.PayNote=payNote;
					response.CCSource=ccSource;
					FinishEdgeExpressUrlRequest(response);
			...
			public static XWebResponse GetUrlForCreditCardAlias(long patNum,CreditCardSource ccSource,bool isWebPayment,double amount=0,bool doCreateAlias=true) {
				//No need to check RemotingRole;no call to db.
				try {
				...
				string approvalCode,string statusDescription,decimal amount,bool doShowSignatureLine,long clinicNum)
			{
				...
				if(ListTools.In(transType,EdgeExpressTransType.CreditReturn,EdgeExpressTransType.CreditVoid)) {

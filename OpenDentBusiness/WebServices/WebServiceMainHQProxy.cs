...
namespace OpenDentBusiness {
	public static class WebServiceMainHQProxy {
		...
		private static bool SyncHostedEmail(EServiceSetup.SignupOut signupOut,IWebServiceMainHQ webServiceMainHq) {
			...
			List<Action> listHostedEmailCredentialCreations=listClinicNumsNeedingSignups.Distinct().Select(clinicNum => new Action(() => {
			...
			if(listHostedEmailCredentialCreations.Count>0) {
				ODThread.RunParallel(listHostedEmailCredentialCreations);
			}

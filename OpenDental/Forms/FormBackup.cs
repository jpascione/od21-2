	...
	public partial class FormBackup : FormODBase {		
      ...
      CDT.Class1.Encrypt(textArchivePass.Text,out encryptedPassword);
		...
		private long GetFileSizes(string fromPath,string toPath){
			...
			for(int i=0;i<fileInfoArray.Length;i++){
				if(!File.Exists(ODFileUtils.CombinePaths(toPath,fileInfoArray[i].Name))){
		...
		private void CopyDirectoryIncremental(string fromPath,string toPath,double amtCopied, double maxSize){
			...
			for(int i=0;i<directoryInfoArray.Length;i++){
				...
				CopyDirectoryIncremental(ODFileUtils.CombinePaths(directoryInfoArray[i].FullName,""),
					ODFileUtils.CombinePaths(destinationPath,""),amtCopied,maxSize);
			}
			FileInfo[] fileInfoArray=directoryInfo.GetFiles();//of fromPath
			for(int i=0;i<fileInfoArray.Length;i++){
				string fromFile=fileInfoArray[i].FullName;
				...
				amtCopied+=(double)fileInfoArray[i].Length/1048576.0; //Number of megabytes.
				string progressBarMessage=Lan.g(this,"Copied ")+Math.Round(amtCopied,2).ToString()+"MB "+Lan.g(this,"of ")+maxSize.ToString()+"MB";
				ProgressBarHelper progressBarHelper=new ProgressBarHelper(progressBarMessage,blockValue:(int)(amtCopied/maxSize*100),blockMax:100);
				ProgressBarEvent.Fire(ODEventType.ProgressBar,progressBarHelper);
			}
		}

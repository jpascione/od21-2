...
namespace OpenDental{
	...
	public partial class FormCarriers : FormODBase {
		...
		private void butCombine_Click(object sender, System.EventArgs e) {
			...
			List<List<InsPlan>> listListsInsPlans=new List<List<InsPlan>>();
			string carrierTo=Carriers.GetName(formCarrierCombine.PickedCarrierNum);
			for(int i = 0;i<pickedCarrierNums.Count;i++) {
				carrierNames.Add(Carriers.GetName(pickedCarrierNums[i]));
				listListsInsPlans.Add(InsPlans.GetAllByCarrierNum(pickedCarrierNums[i]));
			}
			try {
			...
			for(int i=0;i<listListsInsPlans.Count;i++) {
				for(int j=0;j<listListsInsPlans[i].Count;j++) {
					SecurityLogs.MakeLogEntry(Permissions.InsCarrierCombine,0,Lan.g(this,"Carrier with name ")+" "+carrierNames[i]+" "
						+Lan.g(this,"was combined with")+" "+carrierTo,listListsInsPlans[i][j].PlanNum,listListsInsPlans[i][j].SecDateTEdit);
					if(carrierNames[i].Trim().ToLower()!=carrierTo.Trim().ToLower()) {
						SecurityLogs.MakeLogEntry(Permissions.InsPlanChangeCarrierName,0,Lan.g(this,"Carrier with name ")+" "+carrierNames[i]+" "
							+Lan.g(this,"was merged with")+" "+carrierTo,listListsInsPlans[i][j].PlanNum,listListsInsPlans[i][j].SecDateTEdit);
					}
				}
			}
			DataValid.SetInvalid(InvalidType.Carriers);

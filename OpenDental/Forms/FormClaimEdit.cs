	...
	public partial class FormClaimEdit : FormODBase {
      ...
      _listClaimProcs=_loadData.ListClaimProcs;
		...
		private void FillGrids(bool doRefreshData=true){
			...
			GridRow row;
			_listClaimProcsForClaim=ClaimProcs.RefreshForClaim(_claimCur.ClaimNum,_listProcedures,_listClaimProcs);//Excludes labs.
		...
		private void butPayTotal_Click(object sender, System.EventArgs e) {
			...
			bool hasMadePayment;
			if(PrefC.GetBool(PrefName.ClaimPayByTotalSplitsAuto)) {//Determines if the user prefers line item accounting.
		...
		private bool PayAsTotalToClaimProcsExplicitly() { 
			...
			if(_blueBookEstimateData==null) {
				_blueBookEstimateData=new BlueBookEstimateData(_listInsPlans,_listInsSub,_listPatPlans);
			}
			List<ClaimProc> listClaimProcs=new List<ClaimProc>();
			for(int i=0;i<_listClaimProcsForClaim.Count;i++) {
				listClaimProcs.Add(_listClaimProcsForClaim[i].Copy());
			}
			using FormClaimPayTotal formClaimPayTotal=new FormClaimPayTotal(_patientCur,_familyCur,_listInsPlans,_listPatPlans,_listInsSub,_blueBookEstimateData,totalPayAmt:result);
		...
		private void butPayProc_Click(object sender, System.EventArgs e) {
			...
			if(listClaimProc.Count > 0) {
				if(_claimCur.ClaimType=="PreAuth") {
					...
					using FormClaimPayTotal formClaimPayTotal=new FormClaimPayTotal(_patientCur,_familyCur,_listInsPlans,_listPatPlans,_listInsSub,_blueBookEstimateData);
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private void SendClaim() {
			...
			if(clearinghouseHq.Eformat==ElectronicClaimFormat.Canadian) {
				try {
					Canadian.SendClaim(clearinghouseClin,listClaimSendQueueItemsCA[0],true,false,FormCCDPrint.PrintCCD,ShowProviderTransferWindow,
				...
				for(int i=0;i<_listClaimProcsForClaim.Count;i++) {
					...
					if(patPlan.Ordinal==1) {
						ClaimProcs.ComputeBaseEst(_listClaimProcsForClaim[i],procedure,insPlan,patPlan.PatPlanNum,listBenefit,null,null,_listPatPlans,0,0,_patientCur.Age,0
							,_listInsPlans,_listInsSub,ListSubstitutionLinks,false,null,blueBookEstimateData:_blueBookEstimateData);
					}
					else {
						...
						ClaimProcs.ComputeBaseEst(_listClaimProcsForClaim[i],procedure,insPlan,patPlan.PatPlanNum,listBenefit,null,null,_listPatPlans,paidOtherInsTotal
							,paidOtherInsTotal,_patientCur.Age,writeOffOtherIns,_listInsPlans,_listInsSub,ListSubstitutionLinks,false,null,blueBookEstimateData:_blueBookEstimateData);
					}
					InsBlueBookLog insBlueBookLog=_blueBookEstimateData.CreateInsBlueBookLog(_listClaimProcsForClaim[i]);
		...
		private void FormClaimEdit_Closing(object sender,System.ComponentModel.CancelEventArgs e) {
            ...
            if(_claimCur.ClaimStatus!=Claims.GetClaim(_claimCur.ClaimNum).ClaimStatus) {

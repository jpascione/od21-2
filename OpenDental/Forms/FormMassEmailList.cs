	...
	public partial class FormMassEmailList:FormODBase {
		...
		private List<long> GetPatNumsFromUserQuery() {
			...
			if(rgxBlacklist.IsMatch(command)) {
				MsgBox.Show("Query cannot include any keywords that would alter the database.");
				return listPatNums;
			}
		...
		private void UpdateClinicIsNotEnabled() {
			...
			if(template is null) {
				labelNotEnabled.Visible=!comboClinicPatient.ListSelectedClinicNums.Any(x => Clinics.IsMassEmailEnabled(x));
			}
			else {
				labelNotEnabled.Visible=!Clinics.IsMassEmailEnabled(template.ClinicNum);
			}
		}
		...
		private void butPreviewTemplate_Click(object sender,EventArgs e) {
			...
			formWebBrowser.browser.DocumentText=comboEmailHostingTemplate.GetSelected<EmailHostingTemplate>().BodyHTML;

...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlMarketingEdit:UserControl {
		//FIELDS
		public bool IsNew;
		...
		//FUNCTIONS
		public UserControlMarketingEdit() {
			InitializeComponent();
		...
		public void LoadJob(Job job,TreeNode treeNode) {
			...
			//Design Jobs must have a quote attached on creation.
		...
		private void CheckPermissions() {
			...
			switch(_jobCur.PhaseCur) {
				...
				case JobPhase.Documentation:
					if(!JobPermissions.IsAuthorized(JobPerm.DesignCoordinator,true)) {
						break;
					}
					comboPriority.Enabled=true;
					textEditorMain.ReadOnly=false;
		...
		private void butActions_Click(object sender,EventArgs e) {
			...
			switch(_jobCur.PhaseCur) {
				...
				case JobPhase.Definition:
				case JobPhase.Quote:
					perm=JobPermissions.IsAuthorized(JobPerm.DesignTech,true);
					...
					if(JobPermissions.IsAuthorized(JobPerm.MarketingManager,true)) {
					...
					bool hasCompleteReview = _jobCur.ListJobReviews.Exists(x => x.ReviewStatus==JobReviewStatus.Done)&&perm;//Has DesignTech perm
					...
					if(perm) {
						...
						actionMenu.MenuItems.Add(new MenuItem("Cancel Design Job",actionMenu_CancelJobClick));
					}
					break;
				case JobPhase.Development:
					perm=JobPermissions.IsAuthorized(JobPerm.MarketingManager,true);
					actionMenu.MenuItems.Add(new MenuItem((_jobCur.UserNumConcept==0 ? "A" : "Rea")+"ssign Creator",actionMenu_AssignSubmitterClick) { Enabled=perm });
					actionMenu.MenuItems.Add(new MenuItem("Send to Definition",actionMenu_SendDefinitionClick) { Enabled=perm });
					actionMenu.MenuItems.Add(new MenuItem((_jobCur.UserNumEngineer==0 ? "A" : "Rea")+"ssign Owner",actionMenu_AssignEngineerClick) { Enabled=perm||JobPermissions.IsAuthorized(JobPerm.DesignCoordinator,true) });//x
					actionMenu.MenuItems.Add(new MenuItem("Send to Quote",actionMenu_RequestQuoteClick) { Enabled=perm });//x
					...
					actionMenu.MenuItems.Add(new MenuItem("Send to Development",actionMenu_SendInDevelopmentClick) { Enabled=perm });//x
					hasCompleteReview=_jobCur.ListJobReviews.Exists(x => x.ReviewStatus==JobReviewStatus.Done&&JobPermissions.IsAuthorized(JobPerm.DesignTech,true));//Has DesignTech perm
					actionMenu.MenuItems.Add(new MenuItem("Mark as Complete",actionMenu_ImplementedClick) { Enabled=hasCompleteReview });
					if(perm) {
						actionMenu.MenuItems.Add("-");
						actionMenu.MenuItems.Add(new MenuItem("Cancel Design Job",actionMenu_CancelJobClick));
		...
		private void actionMenu_SendDefinitionClick(object sender,EventArgs e) {
			...
			if(!PickUserByJobPermission("Pick Owner",JobPerm.DesignCoordinator,out userNumEngineer,_jobCur.UserNumEngineer,JobPermissions.IsAuthorized(JobPerm.DesignCoordinator,true,Security.CurUser.UserNum),false)) {
				return;
			}
		...
		private void actionMenu_SendInDevelopmentClick(object sender,EventArgs e) {
			...
			long userNumEngineer = _jobCur.UserNumEngineer;
			if(_jobCur.UserNumEngineer==0 && !PickUserByJobPermission("Pick Owner",JobPerm.DesignCoordinator,out userNumEngineer,_jobCur.UserNumEngineer>0 ? _jobCur.UserNumEngineer : _jobCur.UserNumConcept,true,false)) {
				return;
			}
			_jobCur.UserNumEngineer=userNumEngineer;
		...
		private void actionMenu_AssignSubmitterClick(object sender,EventArgs e) {
			...
			if(!PickUserByJobPermission("Pick Creator",JobPerm.DesignTech,out userNumConcept,_jobCur.UserNumConcept,false,false)) {
				return;
			}
		...
		private void actionMenu_AssignExpertClick(object sender,EventArgs e) {
			...
			if(!PickUserByJobPermission("Pick Reviewer",JobPerm.MarketingManager,out userNumExpert,_jobCur.UserNumExpert,true,false)) {
				return;
			}
		...
		private void actionMenu_AssignEngineerClick(object sender,EventArgs e) {
			...
			if(!PickUserByJobPermission("Pick Owner",JobPerm.DesignCoordinator,out userNumEngineer,_jobCur.UserNumEngineer,true,false)) {
				return;
			}
		...
		private void SaveJob(Job job) {
			...
			if(job.JobNum==0 || IsNew) {
				if(job.UserNumConcept==0 && JobPermissions.IsAuthorized(JobPerm.DesignTech,true)) {
		...
		private void gridAppointments_TitleAddClick(object sender,EventArgs e) {
			...
			if(_jobCur.ListJobQuotes.FirstOrDefault().PatNum==0) {
				MsgBox.Show(this,"Customer must be attached to this job before attaching an appointment");
				return;
			}
		...
		private void gridReview_TitleAddClick(object sender,EventArgs e) {
			...
			if(!PickUserByJobPermission("Pick Reviewer",JobPerm.MarketingManager,out userNumReviewer,_jobCur.UserNumExpert,false,false)) {
				return;
			}

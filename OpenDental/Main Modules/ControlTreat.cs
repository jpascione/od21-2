    ...
    private ArrayList ALPreAuth;
      ...
      FillPreAuth();
		...
		private void FillPlans(bool doRefreshData=true){
			...
			List<MobileAppDevice> listMobileAppDevices=MobileAppDevices.GetAll(PatCur.PatNum);
			...
			if(doRefreshData) {
				_tpData.DiscountPlanSub=DiscountPlanSubs.GetSubForPat(PatCur.PatNum);
			}
			...
			for(int i=0;i<_listTreatPlans.Count;i++){
				...
				if(treatPlanCur.MobileAppDeviceNum>0 
					&& listMobileAppDevices.FirstOrDefault(x => x.MobileAppDeviceNum==treatPlanCur.MobileAppDeviceNum)?.PatNum==treatPlanCur.PatNum) 
				{
					row.Cells.Add("X");
				}
				else {
		...
		private void FillSummary(){
			...
			if(_tpData.DiscountPlanSub!=null) {
    ...
    private void FillPreAuth(){
      ...
      for(int i=0;i<ALPreAuth.Count;i++){
		...
		private void gridPlans_CellClick(object sender, OpenDental.UI.ODGridClickEventArgs e) {
			...
			if(gridPlans.ListGridColumns[e.Col].Heading=="eClipboard") {
		...
		private List<ProcTP> LoadActiveTP(ref TreatPlan treatPlan) {
			...
			if(_tpData.DiscountPlanSub!=null && !listProcForTP.IsNullOrEmpty()) {
				listDiscountPlanProcs=DiscountPlans.GetDiscountPlanProc(listProcForTP,discountPlanSub:_tpData.DiscountPlanSub,discountPlan:_tpData.DiscountPlan);
			}
			for(int i=0;i<listProcForTP.Count;i++) {
		...
		private void ToolBarMainUpdate_Click() {
			...
			long discountPlanNum=_tpData.DiscountPlanSub?.DiscountPlanNum??0;
			...
			OrthoSchedule orthoSchedule=null;
			List<OrthoProcLink> listOrthoProcLinksForOrthoCase=null;
			List<DiscountPlanProc> listDiscountPlanProc=DiscountPlans.GetDiscountPlanProc(ProcListTP.ToList(),_tpData.DiscountPlanSub,_tpData.DiscountPlan);
			for(int i=0;i<ProcListTP.Length;i++) {
				...
				if(procOld.ProcFee!=procCur.ProcFee || procOld.TaxAmt!=procCur.TaxAmt || procOld.DiscountPlanAmt!=procCur.DiscountPlanAmt) {

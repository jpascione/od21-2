...
namespace OpenDental{
    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		public void FormOpenDentalShown() {
			...
			string domainUser="";
			if(ArrayCommandLineArgs.Length!=0) {
				for(int i = 0;i<ArrayCommandLineArgs.Length;i++) {
					if(ArrayCommandLineArgs[i].StartsWith("UserName=") && ArrayCommandLineArgs[i].Length>9) {
						odUser=ArrayCommandLineArgs[i].Substring(9).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("PassHash=") && ArrayCommandLineArgs[i].Length>9) {
						odPassHash=ArrayCommandLineArgs[i].Substring(9).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("WebServiceUri=") && ArrayCommandLineArgs[i].Length>14) {
						webServiceUri=ArrayCommandLineArgs[i].Substring(14).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("WebServiceIsEcw=") && ArrayCommandLineArgs[i].Length>16) {
						if(ArrayCommandLineArgs[i].Substring(16).Trim('"')=="True") {
					...
					if(ArrayCommandLineArgs[i].StartsWith("OdPassword=") && ArrayCommandLineArgs[i].Length>11) {
						odPassword=ArrayCommandLineArgs[i].Substring(11).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("ServerName=") && ArrayCommandLineArgs[i].Length>11) {
						serverName=ArrayCommandLineArgs[i].Substring(11).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("DatabaseName=") && ArrayCommandLineArgs[i].Length>13) {
						databaseName=ArrayCommandLineArgs[i].Substring(13).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("MySqlUser=") && ArrayCommandLineArgs[i].Length>10) {
						mySqlUser=ArrayCommandLineArgs[i].Substring(10).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("MySqlPassword=") && ArrayCommandLineArgs[i].Length>14) {
						mySqlPassword=ArrayCommandLineArgs[i].Substring(14).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("MySqlPassHash=") && ArrayCommandLineArgs[i].Length>14) {
						mySqlPassHash=ArrayCommandLineArgs[i].Substring(14).Trim('"');
					}
					if(ArrayCommandLineArgs[i].StartsWith("IsSilentUpdate=") && ArrayCommandLineArgs[i].Length>15) {
						if(ArrayCommandLineArgs[i].Substring(15).Trim('"').ToLower()=="true") {
							isSilentUpdate=true;
						}
						else {
					...
					if(ArrayCommandLineArgs[i].StartsWith("DynamicMode=")) {
						useDynamicMode=ArrayCommandLineArgs[i].ToLower().Contains("true");
					}
					if(ArrayCommandLineArgs[i].StartsWith("DomainUser=")) {
						domainUser=ArrayCommandLineArgs[i].Substring(11).Trim('"');
					}
				}
			}
			...
			while(true) {//Most users will loop through once.  If user tries to connect to a db with replication failure, they will loop through again.
				if(chooseDatabaseInfo.NoShow==YN.Yes) {
					try {
						CentralConnections.TryToConnect(chooseDatabaseInfo.CentralConnectionCur,chooseDatabaseInfo.DbType,
							...
							isCommandLineArgs: (ArrayCommandLineArgs.Length!=0),useDynamicMode: chooseDatabaseInfo.UseDynamicMode);
					}
					catch(Exception) {
						if(isSilentUpdate) {
				...
				if(ArrayCommandLineArgs.Length==0 //eCW doesn't load splash screen
			...
			ProcessCommandLine(ArrayCommandLineArgs);
			ODException.SwallowAnyException(() => {
			...
			if(_strCmdLineShow=="searchpatient") {
				...
				if(formPatientSelect.DialogResult==DialogResult.OK) {
					...
					if(controlChart.Visible) {
						controlChart.ModuleSelectedErx(CurPatNum);
					}
					else {
			...
			if(!PrefC.IsODHQ) {
				...
				_menuItemAccount.DropDown.Items.Clear();
				_menuItemAccount.Click+=menuItemPreferencesAccount_Click;
			}
			...
			LayoutManager.LayoutFormBoundsAndFonts(this);
			LayoutControls();
			if(ArrayCommandLineArgs.Contains("--runLoadSimulation")) {
				StartLoadSimulation();
			}
		...
		private bool PrefsStartup(bool isSilentUpdate,ChooseDatabaseInfo chooseDatabaseInfo){
			...
			if(DataConnection.DBtype==DatabaseType.MySql) {
				...
				if(updateComputerName != "" && ODEnvironment.MachineName.ToUpper() != updateComputerName.ToUpper()) {
					if(isSilentUpdate) {
		...
		private void ResizeDashboard() {
			...
			splitContainerNoFlickerDashboard.SplitterDistance=Math.Max(splitContainerNoFlickerDashboard.Width-splitContainerNoFlickerDashboard.SplitterWidth-width,0);
		...
		private void TryNonPatientPopup() {
			if(CurPatNum!=0 && _patNumPrevious!=CurPatNum) {
				_datePopupDelay=DateTime.Now;
				_patNumPrevious=CurPatNum;
			}
		...
		private void toolButWebMail_Click() {
			...
			using FormWebMailMessageEdit FormWMME=new FormWebMailMessageEdit(CurPatNum);
			FormWMME.ShowDialog();
		}
		...
		private long GetAptNumForLetter() {
			Appointment[] arrayAppts=Appointments.GetForPat(CurPatNum);
			long aptNum;
			if(arrayAppts.Length==0) {
				...
				using FormApptsOther FormA=new FormApptsOther(CurPatNum,null);
		...
		private void DataValid_BecameInvalid(OpenDental.ValidEventArgs e) {
			...
			if(e.ITypes.Contains(InvalidType.Task) || e.ITypes.Contains(InvalidType.TaskPopup)) {
				...
				if(controlChart?.Visible??false) {
					...
					controlChart.ModuleSelected(CurPatNum);
				}
		...
		private void FillSignalButtons(List<SigMessage> listSigMessages) {
			...
			if(_arraySigButDefs==null) {
				_arraySigButDefs=SigButDefs.GetByComputer(SystemInformation.ComputerName);
			}
		...
		private void SignalsTickExceptionHandler(Exception ex) {
			...
			if(_exceptionSignalsTick==null) {
				...
				ODException.SwallowAnyException(() => {
					BugSubmissions.SubmitException(_exceptionSignalsTick,patNumCur: CurPatNum,moduleName: GetSelectedModuleName());
				});
			}
		}
		...
		private void GotoModule_ModuleSelected(ModuleEventArgs e){
			...
			if(e.ClaimNum>0){
				...
				controlAccount.ModuleSelected(CurPatNum,e.ClaimNum);
			}
			else if(e.ListPinApptNums.Count!=0){
				moduleBar.SelectedModule=e.ModuleType;
				controlAppt.Visible=true;
				this.ActiveControl=this.controlAppt;
				controlAppt.ModuleSelectedWithPinboard(CurPatNum,e.ListPinApptNums,e.DateSelected);
			}
				...
				if(PrefC.GetBoolSilent(PrefName.ImagesModuleUsesOld2020,false)){
					...
					controlImages.ModuleSelected(CurPatNum,e.DocNum);
				}
				else{
					moduleBar.SelectedModule=e.ModuleType;
					...
					controlImagesJ.ModuleSelected(CurPatNum,e.DocNum);
				}
			}
		...
		private void SetModuleSelected(bool menuBarClicked){
			switch(moduleBar.SelectedModule){
				case EnumModuleType.Appointments:
					...
					controlAppt.Visible=true;
					this.ActiveControl=this.controlAppt;
					controlAppt.ModuleSelected(CurPatNum);
					break;
				case EnumModuleType.Family:
				...
				case EnumModuleType.Account:
					...
					controlAccount.Visible=true;
					this.ActiveControl=this.controlAccount;
					controlAccount.ModuleSelected(CurPatNum);
					break;
				case EnumModuleType.TreatPlan:
					...
					if(menuBarClicked) {
						controlTreat.ModuleSelected(CurPatNum,true);//Set default date to true when button is clicked.
					}
					else {
						controlTreat.ModuleSelected(CurPatNum);
					}
					break;
				case EnumModuleType.Chart:
					controlChart.InitializeOnStartup();
					...
					if(menuBarClicked) {
						...
						controlChart.ModuleSelected(CurPatNum,true);
					}
					TryNonPatientPopup();
					break;
				case EnumModuleType.Imaging:
					if(PrefC.GetBoolSilent(PrefName.ImagesModuleUsesOld2020,false)){
						...
						controlImages.Visible=true;
						this.ActiveControl=this.controlImages;
						controlImages.ModuleSelected(CurPatNum);
					}
					else{
						controlImagesJ.InitializeOnStartup();
						controlImagesJ.Visible=true;
						this.ActiveControl=this.controlImagesJ;
						controlImagesJ.ModuleSelected(CurPatNum);
					}
					break;
				case EnumModuleType.Manage:
					...
					controlManage.ModuleSelected(CurPatNum);
					break;
			}
		}
		...
		private void RefreshCurrentModule(bool hasForceRefresh=false,bool isApptRefreshDataPat=true,bool isClinicRefresh=true,long docNum=0){
			if(controlAppt.Visible){
				if(hasForceRefresh) {
					...
					if(isApptRefreshDataPat) {//don't usually skip data refresh, only if CurPatNum was set just prior to calling this method
						controlAppt.RefreshModuleDataPatient(CurPatNum);
					}
					controlAppt.RefreshModuleScreenButtonsRight();
				}
			}
			...
			if(controlChart.Visible){
				controlChart.ModuleSelected(CurPatNum,isClinicRefresh);
			}
			if(PrefC.GetBoolSilent(PrefName.ImagesModuleUsesOld2020,false)){
				if(controlImages.Visible){
					controlImages.ModuleSelected(CurPatNum,docNum);
				}
			}
			else{
				if(controlImagesJ.Visible){
					controlImagesJ.ModuleSelected(CurPatNum,docNum);
				}
			}
			if(controlManage.Visible){
				controlManage.ModuleSelected(CurPatNum);
		...
		private void phonePanel_GoToChanged(object sender,EventArgs e) {
			if(_formPhoneTiles.PatNumGoTo!=0) {
				CurPatNum=_formPhoneTiles.PatNumGoTo;
				Patient pat=Patients.GetPat(CurPatNum);
				RefreshCurrentModule();
				FillPatientButton(pat);
			}
		...
		private void menuItemDatabaseMaintenancePat_Click(object sender,EventArgs e) {
			...
			using FormDatabaseMaintenancePat FormDMP=new FormDatabaseMaintenancePat(CurPatNum);
			FormDMP.ShowDialog();
		}
		...
		private void menuItemMassEmails_Click(object sender,EventArgs e) {
			using FormMassEmails formMassEmails=new FormMassEmails();
			formMassEmails.ShowDialog();
		}
		...
		private void RefreshMenuDashboards() {
			...
			this.InvokeIfRequired(() => {
				...
				MenuItemOD menuItem=new MenuItemOD("Dashboard Setup",OpenDashboardSetup);
		...
		private void FormOpenDentalClosing(object sender,FormClosingEventArgs e) {
			...
			try {
				if(!CompareDouble.IsEqual(HelpButtonXAdjustment,ComputerPrefs.LocalComputer.HelpButtonXAdjustment)) {

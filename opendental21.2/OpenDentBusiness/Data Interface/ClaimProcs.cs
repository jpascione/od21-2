...
namespace OpenDentBusiness{
	...
	public class ClaimProcs{
		...
		///<summary>Centralized logic for computing sales tax. Returns whatever value is already there for ODHQ.</summary>
		public static double ComputeSalesTax(Procedure proc,List<ClaimProc> listClaimProcs) {
			if(PrefC.IsODHQ) {
				return proc.TaxAmt;
			}
			if(!ProcedureCodes.GetProcCode(proc.CodeNum).IsTaxed) {
				return 0;
			}
		...
		public static void ComputeBaseEst(ClaimProc cp,Procedure proc,InsPlan plan,long patPlanNum,List<Benefit> benList,List<ClaimProcHist> histList
			...
			Lookup<FeeKey2,Fee> lookupFees,bool doCheckCanadianLabs=true,BlueBookEstimateData blueBookEstimateData=null)
		{
			//No need to check RemotingRole; no call to db.
			...
			if(listProcClaimProcs.Count > 0) {//There exists 1 or 2 estimates for the current lab proc.
				listProcClaimProcs.ForEach(claimProcForLab => {
					...
					if(claimProcForLab.NoBillIns) {
						...
						claimProcForLab.BaseEst=estAmt;
						claimProcForLab.InsEstTotal=estAmt;

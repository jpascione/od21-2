...
namespace OpenDentBusiness {
	...
	public class HieQueues{
		...
		public static string ProcessQueues() {
			...
			List<HieQueue> listHieQueuesToProcess=GetAll();
			if(listHieQueuesToProcess.IsNullOrEmpty()) {
			...
			for(int i=0;i<listHieQueuesToProcess.Count;i++) {
				try {
					...
					if(!ODInitialize.IsRunningInUnitTest) {//Don't create ccd export if running Unit Tests
						...
						string ccdTextForPat=EhrCCD.GenerateSummaryOfCare(patCur);
			...
			if(!listPatNumsFiltered.IsNullOrEmpty()) {
				...
				listHieQueueNumsToDeleted
					.AddRange(listHieQueuesToProcess.Where(x => ListTools.In(x.PatNum,listPatNumsFiltered)).Select(x => x.HieQueueNum).ToList());
			}
			if(!listPatNumsProcessed.IsNullOrEmpty()) {
				...
				listHieQueueNumsToDeleted
					.AddRange(listHieQueuesToProcess.Where(x => ListTools.In(x.PatNum,listPatNumsProcessed)).Select(x => x.HieQueueNum).ToList());
			}

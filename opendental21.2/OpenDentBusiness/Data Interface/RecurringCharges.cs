...
namespace OpenDentBusiness {
	...
	public class RecurringChargerator {
			...
			try {
				...
				foreach(RecurringChargeData chargeData in listRecurringChargeData) {
					...
					if(useEdgeExpress) {
						...
						SendEdgeExpress(chargeData,doForceDuplicatesThisCharge,strBuilderResultFileXCharge);
					}
					else if(useXCharge) {
		...
		public void SendEdgeExpress(RecurringChargeData chargeData,bool forceDuplicates,StringBuilder strBuilderResultFile) {
			...
			XWebResponse response=EdgeExpress.CNP.ProcessPaymentDirect(chargeData.RecurringCharge.PatNum,Lans.g(_lanThis,"Made from automated recurring charge"),
				chargeData.RecurringCharge.ChargeAmt,chargeData.CCSource,false,chargeData.XChargeToken,forceDuplicates);
			...
			long xWebResponseNum=0)
		{
			...
			if(CompareDouble.IsZero(amount)) {
				...
				listPaySplits.Add(CreateUnearnedSplitForPayment(paymentCur,amount));
			}
			else if(ListTools.In(accountingMode,RigorousAccounting.AutoSplitOnly,RigorousAccounting.EnforceFully)) {
		...
		private List<PaySplit> GetLinkedSplitsForChargeData(Patient patCur,RecurringChargeData recCharge,Payment paymentCur) {
			...
			if(recCharge.PayPlanNum > 0) {
				...
				listPaySplits.AddRange(autoSplit.ListAutoSplits.FindAll(x => x.PayPlanNum==recCharge.PayPlanNum));
			}
			else {
				//Always add the suggested splits because the card has already been charged.
				listPaySplits.AddRange(autoSplit.ListAutoSplits);
				//Only add unearned splits when the credit card has indicated that this is desired behavior.
				if(autoSplit.ListAutoSplits.Any(x => x.UnearnedType > 0) && RecurringCharges.CanChargeWhenNoBal(recCharge.CanChargeWhenNoBal)) {
		...
		private PaySplit CreateUnearnedSplitForPayment(Payment paymentCur,double splitAmt) {
			PaySplit split=new PaySplit {

...
namespace OpenDental {
	public partial class FormArManager:FormODBase {
		...
		private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			GridOD gridPrint=new GridOD() { Location=gridCur.Location,Size=new Size(bounds.Width,gridCur.Height),Title=gridCur.Title };
			gridPrint.BeginUpdate();
			gridPrint.ListGridColumns.Clear();
			gridPrint.ListGridColumns.AddRange(gridCur.ListGridColumns.Select(x => x.Copy()));
			int widthColsAndScroll=gridPrint.ListGridColumns.Sum(x => x.ColWidth)+20;//+20 for vertical scroll bar
			gridPrint.ListGridColumns.ToList().ForEach(x => x.ColWidth=(int)Math.Round((float)x.ColWidth*gridPrint.Width/widthColsAndScroll,MidpointRounding.AwayFromZero));
			gridPrint.ListGridColumns[gridPrint.ListGridColumns.Count-1].ColWidth-=gridPrint.ListGridColumns.Sum(x => x.ColWidth)+20-gridPrint.Width;
			gridPrint.ListGridRows.Clear();
			gridPrint.ListGridRows.AddRange(gridCur.ListGridRows);
			gridPrint.EndUpdate();
			...
			if(!_headingPrinted) {
				...
				g.DrawString(gridPrint.Title,subHeadingFont,Brushes.Black,center-g.MeasureString(gridPrint.Title,subHeadingFont).Width/2,yPos);
				yPos+=20;
				//print today's date
			...
			#endregion
			yPos=gridPrint.PrintPage(g,_pagesPrinted,bounds,_headingPrintH);
			_pagesPrinted++;
			if(yPos==-1) {
				e.HasMorePages=true;
			}

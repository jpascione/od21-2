...
namespace OpenDental {
	public partial class FormEtrans835Edit:FormODBase {
		...
		private void gridClaimDetails_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(claimPaid.IsPatientNameMisMatched(pat)) {
			...
			Etrans835Attaches.CreateForClaim(_x835,claimPaid,claim.ClaimNum,isAttachNeeded,_listAttaches);
			if(claimPaid.IsSplitClaim) {
				...
				claimPaid.GetOtherNotDetachedSplitClaims()
					.ForEach(x => Etrans835Attaches.CreateForClaim(_x835,claimPaid,claim.ClaimNum,isAttachNeeded,_listAttaches));
			}
			if(claimPaid.IsProcessed(_listClaimProcs,_listAttaches)) {
...
#region Add Tracking Status
		...
		private void buttonFindClaimMatch_Click(object sender,EventArgs e) {
			...
			if(listClaimNums!=null) {
				...
				for(int i=0;i<listDetachedPaidClaims.Count;i++) {
					Etrans835Attaches.CreateForClaim(_x835,listDetachedPaidClaims[i],listClaimNums[i],true,_listAttaches);
				}
		...
		private void butAutoProcess_Click(object sender,EventArgs e) {
			...
			try {
				...
				textNote.Text=EraAutomationResult.CreateEtransNote(automationResult.Status,textNote.Text);

	...
	public partial class FormInsPlan : FormODBase {
		...
		private void FillOtherSubscribers() {
			...
			if(countSubs>10000) {//10,000 per Nathan.
				...
				comboLinked.Items.AddRange(listStringSubs.ToArray());
		...
		private void butSearch_Click(object sender,System.EventArgs e) {
			...
			textElectID.Text=formElectIDs.selectedID.PayorID;
			_electIdCur=textElectID.Text;
			FillPayor();
		...
		private void butLabel_Click(object sender,System.EventArgs e) {//TODO: Implement ODprintout pattern
			...
			PrintDocument printDocument=new PrintDocument();//only used to pass printerName
			long patNumCur = _patPlanCur!=null ? _patPlanCur.PatNum : 0;
			if(!PrinterL.SetPrinter(printDocument,PrintSituation.LabelSingle,patNumCur,textCarrier.Text+" insurance plan label printed")) {
				return;
			}
		...
		private void butGetElectronic_Click(object sender,EventArgs e) {
			...
			try {
				...
				if(etrans != null) {
					...
					formEtrans270Edit.benList=_listBenefit;
					if(formEtrans270Edit.ShowDialog()==DialogResult.OK) {
						EB271.SetInsuranceHistoryDates(formEtrans270Edit.ListImportedEbs,_patPlanCur.PatNum,_insSubCur);
						#region Plan Notes
						string patName=Patients.GetNameLF(_patPlanCur.PatNum);
						DateTime dateTimePlanEnd=DateTime.MinValue;
						List<DTP271> listDTP271Dates=formEtrans270Edit.ListDTP;
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(_insPlanCur.PlanType!="" || _insPlanCur.FeeSched!=0) {
		...
		private bool DoShowBluebookDeletionMsgBox() {
			...
			if((_planCurOriginal.FeeSched==0 && comboFeeSched.SelectedIndex!=0)	|| (_planCurOriginal.PlanType=="" && _insPlanCur.PlanType!="")) {

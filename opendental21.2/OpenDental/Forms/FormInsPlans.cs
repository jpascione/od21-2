	...
	public partial class FormInsPlans:FormODBase {
		...
		private void butMerge_Click(object sender,EventArgs e) {
			...
			if(isBluebookOn && planCount > 0) {//Make sure user wants to merge selected plans that are Category Percentage with no fee schedule attached, if applicable.
				if(!MsgBox.Show(this,MsgBoxButtons.YesNo,Lan.g(this,"Plans that have no Fee Schedule and a PlanType of Category Percentage that are merged into "
		...
		private int GetInsBlueBookCount(InsPlan planToMergeTo,InsPlan[] arrayPlansSelected) {
			int planCount=0;
			if(planToMergeTo.FeeSched==0 && planToMergeTo.PlanType=="") {//Plan to merge to is blue book eligible, so no insbluebooks will be deleted.
				return planCount;
			}
			for(int i=0;i<arrayPlansSelected.Length;i++) {
				if(arrayPlansSelected[i].PlanType=="" && arrayPlansSelected[i].FeeSched==0) {//This will exclude the planToMergeTo which is in the array.
					planCount+=1;
				}
			}
			return planCount;

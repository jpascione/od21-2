...
namespace OpenDental {
	public partial class FormMassEmails:FormODBase {
		...
		private void FillClinics() {
			...
			gridClinics.ListGridColumns.Add(col);		
			/* Add these columns back when Secure Email is finished.  And set grid to SelectionMode.OneCell.
		...
		private void UpdateControlVisibility() {
			...
			bool areAnyClinicsActivated=gridClinics.GetTags<Clinic>()
				.Any(x => Clinics.IsMassEmailSignedUp(x.ClinicNum) || Clinics.IsSecureEmailSignedUp(x.ClinicNum));
			groupSetup.Enabled=allowEdit && areAnyClinicsActivated;
			groupMassEmail.Enabled=areAnyClinicsActivated;
		}

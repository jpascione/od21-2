...
///<summary></summary>
	public partial class FormPatientEdit : FormODBase {
		...
		public FormPatientEdit(Patient patientCur,Family familyCur){
			...
			listBoxMedicaidStates.Location=new Point(textMedicaidState.Left,textMedicaidState.Bottom);
			listBoxMedicaidStates.Size=new Size(textMedicaidState.Width,100);
			...
			listBoxStates.Size=new Size(textState.Width,100);
...
;						break;
		...
		private void textMedicaidState_KeyUp(object sender,System.Windows.Forms.KeyEventArgs e) {
			...
			listBoxMedicaidStates.Size=new Size(textMedicaidState.Width,h);
		...
		private void textState_KeyUp(object sender,System.Windows.Forms.KeyEventArgs e) {
			...
			listBoxStates.Size=new Size(textState.Width,h);
		...
		private void butOK_Click(object sender ,System.EventArgs e) {
			...
			if(IsNew && PrefC.HasClinicsEnabled) {
				...
				long selectedBillType=comboBillType.GetSelectedDefNum();
				long clinicDefaultBillType=PIn.Long(ClinicPrefs.GetPrefValue(PrefName.PracticeDefaultBillType,comboClinic.SelectedClinicNum));
				if(selectedBillType!=clinicDefaultBillType) {//New patient's selected default billing type doesn't match the clinic's.
					List<Def> listBillTypes=Defs.GetDefsForCategory(DefCat.BillingTypes,true);
					string selectedBillTypeName=listBillTypes[comboBillType.SelectedIndex].ItemName;
					string clinicDefaultBillTypeName=Defs.GetDef(DefCat.BillingTypes,clinicDefaultBillType,listBillTypes).ItemName;
					...
					if(MessageBox.Show(Lan.g(this,"The selected billing type does not match the selected clinic's default billing type. "+
						"Would you like to change this patient's billing type from ")+selectedBillTypeName+" to "+clinicDefaultBillTypeName+"?","",
						MessageBoxButtons.YesNo)==DialogResult.Yes)
					{
						comboBillType.SetSelectedDefNum(clinicDefaultBillType);
					}
				}
			}

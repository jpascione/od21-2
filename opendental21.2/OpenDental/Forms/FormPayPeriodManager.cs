	...
	public partial class FormPayPeriodManager:FormODBase {
		...
		private void GenerateSemiMonthlyPayPeriods() {
			...
			while(numPeriodsGenerated<numPeriods) {//will add either one or zero items to the list on each pass. zero only on first pass
				...
				else if(dateTimeStartPayPeriod.Day<=period1Day) {//current day is before the first pay period of the month
				...
				else if(dateTimeStartPayPeriod.Day>period1Day && dateTimeStartPayPeriod.Day<=period2Day) {	//after first pay period, before second one.

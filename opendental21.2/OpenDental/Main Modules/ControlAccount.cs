...
namespace OpenDental {
	...
	public partial class ControlAccount:UserControl {
		...
		private void contextMenuAcctGrid_Popup(object sender,EventArgs e) {
			...
			menuItemAddRefundOverpayment.Enabled=true;
		...
		private void labelUnearnedAmt_MouseEnter(object sender,EventArgs e) {
			if(PIn.Decimal(labelUnearnedAmt.Text)>0) {
		...
		private void menuPrepayment_Click(object sender,EventArgs e) {
			...
			if(formPT.ShowDialog()==DialogResult.OK) {
				...
				using FormPayment formPayment=new FormPayment(_patCur,famCur,formPT.ReturnPayment,false);
		...
		private void menuItemAddRefund_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<listSelectedRows.Count;i++) {
				if(table.Rows[listSelectedRows[i]]["PayNum"].ToString()!="0") {
					paymentExisting=Payments.GetPayment((long)table.Rows[listSelectedRows[i]]["PayNum"]);
					break;
				}
			}
			...
			if(menuItemSender==menuItemAddRefundWorkNotPerformed) {
				...
				if(listAdjustmentsAdded.Count>0) {
					using FormAdjMulti formAdjMulti=new FormAdjMulti(_patCur,listAdjustmentsToAdd:listAdjustmentsAdded);
		...
		private void menuItemLimitedCustom_Click(object sender,EventArgs e) {
			...
			using FormLimitedStatementSelect formLSS=new FormLimitedStatementSelect(table,listPayClaimNums,listAdjNums,listPayNums,listProcNums,listPatNums);
		...
		private void TabControlAccount_DrawItem(object sender,DrawItemEventArgs e) {
			...
			if(CompareDouble.IsZero(totPaySplitAmount)) {
				e.Graphics.FillRectangle(new SolidBrush(SystemColors.Control),tabArea);//make the tab grey if hidden splits total $0
				tabPage.ForeColor=SystemColors.ControlText;
			}
			else {
				e.Graphics.FillRectangle(new SolidBrush(Color.Red),tabArea);//make the tab red if hidden splits do not total $0
				tabPage.ForeColor=SystemColors.Window;
			}
			TextRenderer.DrawText(e.Graphics,tabPage.Text,Font,tabArea,tabPage.ForeColor);
		}
		...
		public void InitializeOnStartup() {
          ...
          labelStartDate,
			...
			LayoutToolBar();
			_labelUnearnedBreakdown.Font=new Font(FontFamily.GenericMonospace,LayoutManager.ScaleF(8.25f));
		...
		private void RefreshModuleData(long patNum,bool isSelectingFamily) {
			...
			_listSplitsHidden=_loadData.ListUnearnedSplits
				.FindAll(x => ListTools.In(x.UnearnedType,PaySplits.GetHiddenUnearnedDefNums()) && ListTools.In(x.PatNum,_famCur.GetPatNums()));//Don't show out of family pay splits for the payer.
			FillSummary();
			...
			if(_patCur!=null) {
				...
				for(int i=0;i<tableMisc.Rows.Count;i++){
					if(tableMisc.Rows[i]["descript"].ToString()=="unearnedIncome") {
						...
						List<PaySplit> listUnearnedShownOnAccount=_loadData.ListUnearnedSplits.FindAll(x => !ListTools.In(x.UnearnedType,listDefNumsTpUnearned) 
							&& ListTools.In(x.PatNum,_famCur.ListPats.Select(y => y.PatNum)));//We do not want to show unearned balances for paysplits to other families
		...
		private void FillPaymentPlans() {
			...
			for(int i=0;i<table.Rows.Count;i++) {
				...
				if(table.Rows[i]["IsClosed"].ToString()=="1" && PrefC.GetInt(PrefName.PayPlansVersion)==2) {
					...
					if(ListTools.In(PrefC.GetInt(PrefName.PayPlansVersion),(int)PayPlanVersions.DoNotAge,(int)PayPlanVersions.AgeCreditsOnly,(int)PayPlanVersions.NoCharges)) 
					{
		...
		private bool AddProcAndValidate(string procString,Provider patProv) {
			...
			if(proc.ProcStatus==ProcStat.C) {
				...
				ProcedureL.AfterProcsSetComplete(new List<Procedure>() { proc });
			}
			return true;
		}
		...
		public void LayoutPanels(){
			...
			if(_listSplitsHidden.Count==0) {//might need to get updated more often than from loadData. Not sure how much we care. 
				LayoutManager.Remove(tabPageHiddenSplits);
			}
			...
			{//if any additional tabs present besides main one
				...
				tabControlAccount.ItemSize=LayoutManager.ScaleSize(new Size(370,18));
				LayoutManager.Move(tabControlAccount,new Rectangle(-4,-3,gridComm.Width+8,splitContainerAccountCommLog.Panel1.Height+7));
			}
			else {
				tabControlAccount.Appearance=TabAppearance.FlatButtons;
				tabControlAccount.SizeMode=TabSizeMode.Fixed;
				tabControlAccount.ItemSize=LayoutManager.ScaleSize(new Size(0,1));
				//make the tab control artificially large to get rid of the 3D border effect.
				LayoutManager.Move(tabControlAccount,new Rectangle(-4,-5,gridComm.Width+8,splitContainerAccountCommLog.Panel1.Height+9));
			}
		...
		private void AddAdjustmentToSelectedProcsHelper(bool openMultiAdj=false) {
			...
			if(listSelectedProcs.Count>1 || openMultiAdj) {
				...
				using FormAdjMulti formAdjMulti=new FormAdjMulti(_patCur,listSelectedProcs);

...
namespace OpenDental {
	...
	public partial class ControlChart:UserControl	{
		...
		private void butErxAccess_Click(object sender,EventArgs e) {
			using FormErxAccess formErxAccess=new FormErxAccess(_patCur);
		...
		private void ChartViewsDoubleClicked(ODGridClickEventArgs e) {
			...
			if(gridChartViews.ListGridRows.Count==count) {
				...
				if(formChartView.ChartViewCur.ItemOrder!=0) {
					gridChartViews.SetSelected(e.Row-1,true);
					SetChartView(_listChartViews[e.Row-1]);
				}
				else {
		...
		private void gridPtInfo_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(gridPtInfo.ListGridRows[e.Row].Tag!=null && gridPtInfo.ListGridRows[e.Row].Tag.ToString()!="DOB") {
				if(new[] { "tabMedical","tabProblems","tabMedications","tabAllergies","tabTobaccoUse" }.Contains(gridPtInfo.ListGridRows[e.Row].Tag.ToString())) {
					using FormMedical formMedical=new FormMedical(_patientNoteCur,_patCur,gridPtInfo.ListGridRows[e.Row].Tag.ToString());
				...
				if(gridPtInfo.ListGridRows[e.Row].Tag.ToString()=="Patient Portal") {
					using FormPatientPortal formPP=new FormPatientPortal(_patCur);
					formPP.ShowDialog();
		...
		private void listPriorities_MouseClick(object sender,MouseEventArgs e) {
			...
			TreatPlanType treatPlanTypeCur=DiscountPlanSubs.HasDiscountPlan(_patCur.PatNum)?TreatPlanType.Discount:TreatPlanType.Insurance;
		...
		private void menuItemChartBig_Click(object sender,EventArgs e) {
			...
			formTCB.PatCur=_patCur.Copy();
			formTCB.ListAppts=_arrayAppts?.ToList();
			formTCB.Show();
		}
		...
		private void menuItemRxManage_Click(object sender,EventArgs e) {
			using FormRxManage formRxManage=new FormRxManage(_patCur);
		...
		private void menuItemViewMultiVisitGroup_Click(object sender,EventArgs e) {
			...
			List<long> listSelectedProcedures=gridProg.SelectedIndices.Select(x => PIn.Long(((DataRow)gridProg.ListGridRows[x].Tag)["ProcNum"].ToString())).ToList();
			//Get all PMVs from all selected procedures' MV groups
			List<ProcMultiVisit> listSelectedPMVs=ProcMultiVisits.GetGroupsForProcsFromDb(listSelectedProcedures.ToArray());
			List<DataRow> listRawPMVprocs=LoadData.TableProgNotes.Select()
				.Where(x => ListTools.In(PIn.Long((x["ProcNum"].ToString())),listSelectedPMVs.Select(y => y.ProcNum).ToList()))
				.ToList();
			//Get all procedures per PMV group
			Dictionary<long,List<DataRow>> dictProceduresPerGroup=new Dictionary<long,List<DataRow>>();
			for(int i=0;i<listRawPMVprocs.Count;i++) {
				DataRow currentRow=listRawPMVprocs[i];
				if(!dictProceduresPerGroup.ContainsKey(listSelectedPMVs[i].GroupProcMultiVisitNum)) {
					dictProceduresPerGroup.Add(listSelectedPMVs[i].GroupProcMultiVisitNum,new List<DataRow>());
				}
				dictProceduresPerGroup[listSelectedPMVs[i].GroupProcMultiVisitNum].Add(currentRow);
			}
			...
			foreach(KeyValuePair<long,List<DataRow>> keyValuePair in dictProceduresPerGroup) {
				FormMultiVisitGroup formMultiVisitGroup=new FormMultiVisitGroup {
					GridColumns=gridProg.ListGridColumns,
					GridTitle=gridProg.Title,
					ListGridDisplayFields=listDisplayFields,
					ListDataRows=keyValuePair.Value,
					FuncBuildGridRow=(t) => GridProgRowConstruction(t,listDisplayFields),
					ListChartModulePMV=LoadData.ListProcMultiVisits
				};
		...
		private void menuOrthoChart_Click(object sender,EventArgs e) {
			...
			using FormOrthoChart formOrthoChart=new FormOrthoChart(_patCur,orthoChartTabIndex);
      ...
      //Pen penBlue=new Pen(Color.FromArgb(97,136,173));
		...
		private void pd2_PrintPageDay(object sender,PrintPageEventArgs e) {
			...
			if(!_headingPrinted) {
				...
				if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ || CloudStorage.IsCloudStorage) {
					Bitmap bitmapPatPic=Documents.GetPatPict(_patCur.PatNum,ImageStore.GetPatientFolder(_patCur,ImageStore.GetPreferredAtoZpath()));
					if(bitmapPatPic!=null) {
						Bitmap bitmap80=ImageHelper.GetBitmapResized(bitmapPatPic,80);
		...
		private void butAddProc_Click(object sender,EventArgs e) {
			...
			List<Fee> listFees=Fees.GetListFromObjects(new List<ProcedureCode>(){procedureCode },null,//no proc, so medical code won't have changed
				...
				_patCur.PriProv,_patCur.SecProv,_patCur.FeeSched,_listInsPlans,new List<long>(){_patCur.ClinicNum},_arrayAppts.ToList(),_listSubstitutionLinks,discountPlanNum);
			...
			if(ProcedureCodes.GetStringProcCode(formProcCodes.SelectedCodeNum)!="D9986" && ProcedureCodes.GetStringProcCode(formProcCodes.SelectedCodeNum)!="D9987") {
				Procedures.SetDateFirstVisit(DateTime.Today,1,_patCur);
			}
			Procedure procCur;
			...
			switch(ChartModules.GetRowType(row,out long rowPk)){
				...
				case ProgNotesRowType.Rx:
					...
					if(rx==null) {
						...
						using FormRxEdit formRxEdit=new FormRxEdit(_patCur,rx);
				...
				case ProgNotesRowType.EmailMessage:
					...
					if(EmailMessages.IsSecureWebMail(emailMsg.SentOrReceived)) {
						...
						using FormWebMailMessageEdit formWMME=new FormWebMailMessageEdit(_patCur.PatNum,emailMsg);
		...
		private void butPin_Click(object sender,EventArgs e) {
			...
			if(ListTools.In(_patCur.PatStatus,PatientStatus.Archived,PatientStatus.Deceased)) {
				...
				switch(fieldCur.InternalName) {
					...
					case "Broken Appts":
						...
						if(ProcedureCodes.IsValidCode("D9986")) {
							...
							count=Adjustments.GetAdjustForPatByType(_patCur.PatNum,PrefC.GetLong(PrefName.BrokenAppointmentAdjustmentType)).Count;
						}
		...
		private void AttachProcToTPs(Procedure proc) {
			...
			if(_listTreatPlans.All(x => x.TPStatus!=TreatPlanStatus.Active)) {
				TreatPlan activeTreatPlan=new TreatPlan() {
					...
					PatNum=_patCur.PatNum,
					TPType=DiscountPlanSubs.HasDiscountPlan(_patCur.PatNum) ? TreatPlanType.Discount : TreatPlanType.Insurance
				};
		...
		private bool CanChangeProcsStatus(ProcStat newProcStatus,DataRow row,bool doCheckDb,bool isSilent) {
			...
			if(procCur.AptNum!=0) {//if attached to an appointment
				appt=doCheckDb ? Appointments.GetOneApt(procCur.AptNum) 
					//doCheckDb is only false when creating right-click menu.  MenuItem click will use doCheckDb=true.
					: (LoadData.ArrAppts.FirstOrDefault(x => x.AptNum==procCur.AptNum)??new Appointment());
			...
			if(ListTools.In(procOld.ProcStatus,ProcStat.C,ProcStat.EO,ProcStat.EC)) {
		...
		private bool CanCompleteAppointment(bool doCheckDb,bool isSilent) {
			...
			if(!ListTools.In(apt.AptStatus,ApptStatus.PtNote,ApptStatus.PtNoteCompleted)  //PtNote blocked above, added here in case we ever enhance
			...
			ref int skippedLinkedToOrthoCase,ref int skippedAttachedToPreauth,OrthoProcLink orthoProcLink) 
		{
			...
			if(procNum!=0) {
				...
				if(ListTools.In(procStat,ProcStat.C,ProcStat.EC,ProcStat.EO)
				...
				if(ListTools.In(procStat,ProcStat.TP,ProcStat.TPi)) {
			...
			if(procNumCur!=0) {//if this is a procedure 
				...
				if(_patCur!=null&&patNumCur!=_patCur.PatNum) {//if this is a different family member
					...
					if(!checkCommSuperFamily.Checked && PrefC.GetBool(PrefName.ShowFeatureSuperfamilies)) {//Don't show super family members
						if(!_famCur.ListPats.Any(x => x.PatNum==patNumCur)) {//if this is a different super family member (outside family)
							return false;
						}
					}
				}
		...
		private void DrawProcGraphics(bool isPrinting=false) {
			...
			string[] arrayTeeth;
			List<long> listCurProvNums=_toothChartRelay.GetPertinentProvNumsForToothColorPref(Security.CurUser,_patCur,_arrayAppts.ToList());
			for(int i=0;i<_listRowsProcForGraphical.Count;i++) {
				...
				ProcStat curStatus=(ProcStat)PIn.Long(_listRowsProcForGraphical[i]["ProcStatus"].ToString());
				long procProvNum=PIn.Long(_listRowsProcForGraphical[i]["ProvNum"].ToString());
				...
				switch(procCode.PaintType) {
				  ...
				  case ToothPaintingType.FillingLight:
					...
					case ToothPaintingType.RetainedRoot:
						_toothChartRelay.SetRetainedRoot(_listRowsProcForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
					...
					case ToothPaintingType.SpaceMaintainer:
						if(procCode.TreatArea==TreatmentArea.Tooth && _listRowsProcForGraphical[i]["ToothNum"].ToString()!=""){
							...
							if(_listRowsProcForGraphical[i]["Surf"].ToString()=="UR") {
								arrayTeeth=new string[] {"4","5","6","7","8"};
							}
							if(_listRowsProcForGraphical[i]["Surf"].ToString()=="UL") {
								arrayTeeth=new string[] {"9","10","11","12","13"};
							}
							if(_listRowsProcForGraphical[i]["Surf"].ToString()=="LL") {
								arrayTeeth=new string[] {"20","21","22","23","24"};
							}
							if(_listRowsProcForGraphical[i]["Surf"].ToString()=="LR") {
					...
					case ToothPaintingType.Text:
						_toothChartRelay.SetText(_listRowsProcForGraphical[i]["ToothNum"].ToString(),cDark,procCode.PaintText);
						break;
		...
		private void FillToothChart(bool retainSelection,DateTime dateLimit) {
			...
			foreach(DataRow row in _listProcsSkipped) {
				if(ListTools.In(((ProcStat)PIn.Long(row["ProcStatus"].ToString())),ProcStat.C,ProcStat.EO,ProcStat.EC)) {
			...
			foreach(DataRow row in _listRowsProcForGraphical) {
				...
				if(!ListTools.In(procStatus,ProcStat.TP)) {
					continue;
				}
		...
		private void MenuItemSetSelectedProcsStatus(ProcStat newProcStatus) {
			...
			foreach(DataRow row in listSelectedRows) {
				...
				if(procNew.ProcStatus==ProcStat.C) {//Proc set complete.
					...
					//broken appointment procedure codes shouldn't trigger DateFirstVisit update.
					if(ListTools.In(ProcedureCodes.GetStringProcCode(procNew.CodeNum),"D9986","D9987")) {
				...
				if(ListTools.In(procNew.ProcStatus,ProcStat.EC,ProcStat.EO)) {
				...
				if(isProcLinkedToOrthoCase) {//If proc was linked to an ortho case, pass ortho case data to ComputeEstimates.
					...
					Procedures.ComputeEstimates(procNew,procNew.PatNum,ref listClaimProcs,false,_listInsPlans,_listPatPlans,_listBenefits,
						LoadData.ListClaimProcHists,listClaimProcHistsLoop,true,_patCur.Age,_listInsSubs);
				}
			}
		...
		private void PlannedApptPromptHelper() {
			if(_patCur==null || !PrefC.GetBool(PrefName.ShowPlannedAppointmentPrompt) || UsingEcwTightOrFull()) {
				return;
			}
...
//todo: optimize for Full mode:
		...
		private bool ShouldRowShowGraphical(DataRow rowCur,DateTime dateLimit) {
			...
			if(!ListTools.In(procStatus,ProcStat.C,ProcStat.TP) && dateLimit<dateComplete) {//slider date is before the completion date and the procedure is not C or TP
				_listProcsSkipped.Add(rowCur);
				return false;//Skip the proc
			}
			...
			if(erxOption==ErxOption.Legacy) {
				...
				try {
					...
					if(oIdExternal==null) {
						...
						DoseSpot.ValidatePatientData(_patCur);
						string token=DoseSpotREST.GetToken(doseSpotUserID,doseSpotClinicID,doseSpotClinicKey);
						DoseSpotREST.EditPatient(token,_patCur,oIdExternal.IDExternal);
					}
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private void Tool_Perio_Click() {
			...
			if(ToothChartRelay.IsSparks3DPresent) {
				using FormPerio formPerio=new FormPerio(_patCur,null);
				formPerio.ShowDialog();
			}
			else{
				using FormPerio formPerio=new FormPerio(_patCur,listProcedures);
				formPerio.ShowDialog();
			}
		}
		...
		private void Tool_Ortho_Click() {
			...
			using FormOrthoChart formOrthoChart=new FormOrthoChart(_patCur);
			formOrthoChart.ShowDialog();
			...
			if(UsingEcwTightOrFull() && Bridges.ECW.UserId!=0) {
				...
				try {
					...
					webBrowserEcw.Url=new Uri("http://"+strAppServer+"/mobiledoc/jsp/dashboard/Overview.jsp?ptId="
							+_patCur.PatNum.ToString()+"&panelName=overview&pnencid="
							+Bridges.ECW.AptNum.ToString()+"&context=progressnotes&TrUserId="+Bridges.ECW.UserId.ToString());
					labelECWerror.Visible=false;
				}
				catch(Exception ex) {
				...
				using FormRxSelect formRxSelect=new FormRxSelect(_patCur);
		...
		///<summary>Does a fresh layout of the controls, based on the Sheet, the ChartView, </summary>
		...
		private void AddProcedure(Procedure ProcCur,List<Fee> listFees) {
			...
			List<Procedure> listProcedures=Procedures.GetTpForPats(new List<long>(){_patCur.PatNum});
			for(int i=0;i<listProcedures.Count;i++) {
				if(listProcedures[i].ProcNum==ProcCur.ProcNum) {
					break;
				}
				listClaimProcHistsLoop.AddRange(ClaimProcs.GetHistForProc(listClaimProc,listProcedures[i].ProcNum,listProcedures[i].CodeNum));
			}
			...
			if(formProcEdit.DialogResult==DialogResult.Cancel) {
				try {
					...
					LoadData.ListProcs.RemoveAll(x=>x.ProcNum==ProcCur.ProcNum);
				}
				catch(Exception ex) {
					MessageBox.Show(ex.Message);
				}
		...
		private bool AddProcHelper(Procedure ProcCur,List<Fee> listFees) {
			...
			LoadData.ListProcs.Add(ProcCur);//Just in case a data refresh won't been executed
			...
			Procedures.ComputeEstimates(ProcCur,_patCur.PatNum,ref listClaimProcs,true,_listInsPlans,_listPatPlans,_listBenefits,
				LoadData.ListClaimProcHists,listClaimProcHistsLoop,true,_patCur.Age,_listInsSubs,null,false,false,_listSubstitutionLinks,false,
			...
			if(AutoCodeItems.ShouldPromptForCodeChange(ProcCur,procCodeCur,_patCur,isMandibular,listClaimProcs,out verifyCode)) {
				using FormAutoCodeLessIntrusive FormACLI=new FormAutoCodeLessIntrusive(_patCur,ProcCur,procCodeCur,verifyCode,_listPatPlans,_listInsSubs,_listInsPlans,
		...
		private void FillMovementsAndHidden() {
			...
			if(_listToothInitials!=null){
				_listToothInitialsText=_listToothInitials.FindAll(x=>x.InitialType==ToothInitialType.Text);
			}

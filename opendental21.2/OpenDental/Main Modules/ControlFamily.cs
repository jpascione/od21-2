	...
	public class ControlFamily : UserControl {
		...
		private void FillPatientData() {
			if(_patCur==null) {
				...
				_gridPat.ListGridColumns.Clear();
				_gridPat.EndUpdate();
				return;
			}
			...
			_gridPat.ListGridColumns.Add(col);
		...
		private void FillFamilyData() {
			...
			_gridFamily.ListGridColumns.Clear();
			GridColumn col=new GridColumn(Lan.g("TablePatient","Name"),140);
			_gridFamily.ListGridColumns.Add(col);
		...
		private void ToolButDelete_Click() {
			...
			List<Appointment> listAppts=Appointments.GetListForPat(_patCur.PatNum);
		...
		private void FillGridRecall() {
			...
			_gridRecall.ListGridColumns.Clear();
			GridColumn col;
			for(int i=0;i<listDisplayFieldsRecall.Count;i++) {
				...
				_gridRecall.ListGridColumns.Add(col);
			}
		...
		private void FillGridSuperFam() {
			...
			_gridSuperFam.ListGridColumns.Clear();
			GridColumn col=new GridColumn(Lan.g("gridSuperFam","Name"),280);
			_gridSuperFam.ListGridColumns.Add(col);
		...
		private void ToolButAddSuper_Click() {
			...
			else if(ListTools.In(diagResult,DialogResult.None,DialogResult.No)) {//None = the fam doesn't belong to another super fam, just move into this super fam
		...
		private void FillInsData() {
			if(_patCur!=null && _discountPlanSubCur!=null) {
				...
				_gridIns.ListGridColumns.Clear();
				_gridIns.ListGridRows.Clear();
				_gridIns.ListGridColumns.Add(new GridColumn("",170));
				_gridIns.ListGridColumns.Add(new GridColumn(Lan.g(this,"Discount Plan"),170));
			...
			GridColumn col;
			col=new GridColumn("",150);
			_gridIns.ListGridColumns.Add(col);
			int dentalOrdinal=1;
			for(int i=0;i<_listPatPlans.Count;i++) {
			...
			for(int y=0;y<benefitMatrix.GetLength(1);y++) {//rows
				...
				for(int x=0;x<benefitMatrix.GetLength(0);x++) {//columns
					...
					if(benefitMatrix[x,y].BenefitType==InsBenefitType.WaitingPeriod 
						&& ListTools.In(benefitMatrix[x,y].QuantityQualifier,BenefitQuantity.Months,BenefitQuantity.Years))
					{
				...
				foreach(PatPlan patPlan in _listPatPlans) {
					...
					if(dictInsProcHist.TryGetValue(patPlan.InsSubNum,out InsProcHist insProcHist)
						...
						&& insProcHist.ListClaimProcs
							.Exists(x => x.InsSubNum==patPlan.InsSubNum && ListTools.In(x.Status,ClaimProcStatus.InsHist,ClaimProcStatus.Received) && x.ProcNum==proc.ProcNum))
					{
		...
		private void ToolButIns_Click() {
			...
			if(formInsPlan.ShowDialog()!=DialogResult.Cancel) {
				SecurityLogs.MakeLogEntry(Permissions.PatPlanCreate,_patCur.PatNum,"Inserted new PatPlan for patient. InsPlanNum: "+formInsPlan.PlanCurNum);
		...
		private void FillGridPatientClones() {
			...
			_gridPatientClones.ListGridColumns.Clear();
			_gridPatientClones.ListGridColumns.Add(new GridColumn(Lan.g(_gridPatientClones.TranslationName,"Name"),150));
			if(PrefC.HasClinicsEnabled) {
				_gridPatientClones.ListGridColumns.Add(new GridColumn(Lan.g(_gridPatientClones.TranslationName,"Clinic"),80));
			}
			_gridPatientClones.ListGridColumns.Add(new GridColumn(Lan.g(_gridPatientClones.TranslationName,"Specialty"),150){ IsWidthDynamic=true });

	...
	public partial class ControlImages:System.Windows.Forms.UserControl {
		...
		public void SelectTreeNode(TreeNode treeNode,string localPathCloud="") {
			...
			if(nodeIdTag.NodeType==EnumNodeType.Category) {
				...
				_listMountItems=MountItems.GetItemsForMount(nodeIdTag.PriKey);
				_arrayDocumentsInMount=Documents.GetDocumentsForMountItems(_listMountItems);
				_idxSelectedInMount=-1;//No selection to start.
				Action actionCloseDownloadProgress=null;
				if(CloudStorage.IsCloudStorage) {
					actionCloseDownloadProgress=ODProgress.Show(ODEventType.ContrImages,startingMessage:Lan.g("ContrImages","Downloading..."));
				}
				_arrayBitmapsRaw=ImageStore.OpenImages(_arrayDocumentsInMount,_patFolder,localPathCloud);
				actionCloseDownloadProgress?.Invoke();
				_mountShowing=Mounts.GetByNum(nodeIdTag.PriKey);
				_bitmapShowing=new Bitmap(_mountShowing.Width,_mountShowing.Height);
				ImageHelper.RenderMountImage(_bitmapShowing, new List<Bitmap>(_arrayBitmapsRaw), _listMountItems, 
					new List<Document>(_arrayDocumentsInMount), _idxSelectedInMount);
				EnableToolBarButtons(print:true, delete:true, info:true, copy:true, sign:false, brightAndContrast:false, crop:false, hand:true, zoomIn:true, zoomOut:true, flip:false, rotateL:false, rotateR:false, export:true);
			}
			else if(nodeIdTag.NodeType==EnumNodeType.EhrAmend) {
		...
		private void ToolBarCapture_Click() {
			...
			if(ToolBarMain.Buttons["Capture"].Pushed) {
				...
				if(nodeIdTag.NodeType!=EnumNodeType.Mount) {//No mount is currently selected.
					...
					ImageHelper.RenderMountFrames(_bitmapShowing,_listMountItems,-1);
		...
		private void ToolBarInfo_Click() {
			...
			if(nodeIdTag.NodeType==EnumNodeType.Mount) {
				using FormMountEdit form=new FormMountEdit(_mountShowing);
			...
			while(true) {
				try {
					...
					if(_nodeTypeForSettings==EnumNodeType.Doc) {
						...
						ImageHelper.RenderMountFrames(_bitmapShowing,_listMountItems,_idxSelectedInMount);
						...
						if(docForSettings!=null && imageSettingFlags!=ImageSettingFlags.NONE) {
							for(int i=0;i<idxsFlaggedForUpdate.Length;i++) {
								if(idxsFlaggedForUpdate[i]) {
									...
									ImageHelper.RenderImageIntoMount(_bitmapShowing,_listMountItems[i],_arrayBitmapsRaw[i],docForSettings);
								}
							}
						}
						RenderCurrentImage(new Document(),_bitmapShowing.Width,_bitmapShowing.Height,_zoomImage*_zoomOverall,_pointTranslation);

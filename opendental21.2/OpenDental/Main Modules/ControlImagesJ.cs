	...
	public partial class ControlImagesJ : UserControl{
		...
		private void imageSelector_DraggedToCategory(object sender,DragEventArgsOD e) {
			if(e.DocNum!=0){
				...
				string docOldCat=Defs.GetDef(DefCat.ImageCats,GetDocumentShowing0().DocCategory).ItemName;
				string docNewCat=Defs.GetDef(DefCat.ImageCats,e.DefNumNew).ItemName;
					string logText=Lan.g(this,"Document moved")+": "+GetDocumentShowing0().FileName;
				if(GetDocumentShowing0().Description!="") {
					string docDescript=GetDocumentShowing0().Description;
				...
				SecurityLogs.MakeLogEntry(Permissions.ImageEdit,GetDocumentShowing0().PatNum,logText,GetDocumentShowing0().DocNum,GetDocumentShowing0().DateTStamp);
				Document docOld=GetDocumentShowing0().Copy();
				GetDocumentShowing0().DocCategory=e.DefNumNew;

...
namespace OpenDental{
	...
	public class ControlManage : UserControl{
		...
		private GroupBoxOD groupBox1;
		private GroupBoxOD groupBox2;
		private GroupBoxOD groupBox3;
		...
		private void InitializeComponent(){
			...
			this.groupBox2.IsLighter = false;
      ...
      if(_formEmailInbox==null || _formEmailInbox.IsDisposed) {
		...
		private void butImportInsPlans_Click(object sender,EventArgs e) {
			if(_formEtrans834Import!=null && _formEtrans834Import.FormE834P!=null && !_formEtrans834Import.FormE834P.IsDisposed) {
				_formEtrans834Import.FormE834P.Show();
				_formEtrans834Import.FormE834P.BringToFront();
				return;
			}
      ...
      if(PrefC.GetBool(PrefName.LocalTimeOverridesServerTime)) {
		...
		private void FillMessages() {
			...
			for(int i=0;i<gridMessages.ListGridRows.Count;i++) {
				...
				if(ListTools.In(sigMessage.SigMessageNum,listSelectedSigMessageNums)) {

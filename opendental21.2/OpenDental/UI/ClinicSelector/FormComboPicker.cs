...
namespace OpenDental.UI{
	...
	public partial class FormComboPicker : Form{
		...
		private void FormComboPicker_KeyDown(object sender, KeyEventArgs e) {
			...
			if(char.IsLetter((char)e.KeyCode) && !_isMultiSelect) {//alpha character down
				string keyCode=e.KeyCode.ToString();
				if(_listSelectedIndices.Count<1) {
					SetSearchedIndex(0,keyCode,true);
					Invalidate();
					return;
				}
		...
		private bool SetSearchedIndex(int startIdx, string keyCode, bool addIdx=false) {
			for(int i=startIdx;i<ListStrings.Count;i++) {//loop through the items and only add the item if it is found
				if(ListStrings[i].ToUpper().StartsWith(keyCode)) {//keyCode is already uppercased
					if(addIdx) {
						_listSelectedIndices.Add(i);
					}
					else {
